...
# Check if we're in the vermouth git repo
is_in_repo() {
    if [ -f "go.mod" ] && grep -q "vermouth" go.mod 2>/dev/null; then
        if [ -f "build.sh" ]; then
            return 0
        fi
    fi
...
build_local() {
    ...
    BINARY=$(ls build/vermouth-*-${OS}-${ARCH} 2>/dev/null | head -1)
    if [ -z "$BINARY" ]; then
...
download_release() {
    ...
    VERSION="${LATEST#v}"
    FILENAME="vermouth-${VERSION}-${OS}-${ARCH}"
    URL="https://github.com/${REPO}/releases/download/${LATEST}/${FILENAME}"

    # Create temp directory
    TMP_DIR=$(mktemp -d)
    BINARY="${TMP_DIR}/vermouth"
...
install_binary() {
    ...
    if [ -w "$INSTALL_DIR" ]; then
        cp "$BINARY" "$INSTALL_DIR/vermouth"
        chmod +x "$INSTALL_DIR/vermouth"
    else
        sudo cp "$BINARY" "$INSTALL_DIR/vermouth"
        sudo chmod +x "$INSTALL_DIR/vermouth"
    fi
}
...
echo ""
log "${GREEN}✓ Installed vermouth to ${INSTALL_DIR}/vermouth${NC}"
echo ""
echo "Run 'vermouth --help' to get started"
echo ""
